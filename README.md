# Wagih-Soudi
# Hi there! 👋 I'm Wagih Soudi

 Software Quality Assurance Engineer | Test Automation Enthusiast

- 🔭 Currently working as a **QA Engineer**
- 🌱 Learning **Test Automation**, , **Playwright**, and **Cypress**
- 💡 Passionate about delivering high-quality software through effective testing
- 📫 How to reach me: `Wagehmohamedsoudi1@gmail.com`
- 📍 Based in Cairo, Egypt

---

 🛠️ Skills & Tools

- Manual Testing • Test Case Design** • Bug Reporting**
- Automation Testing: Selenium • Playwright • Cypress
- Programming: Python • JavaScript
---

 📌 Featured Projects

- [Llumen] - The Decision Interface
Llumen creates a direct path from data to understanding. It unifies context, visualization, and AI so leaders can see what matters, explore it effortlessly, and move with confidence when decisions need to be made.

---

 📊 Certifications
- ISTQB Foundation Level from Bisics to certificate ITI Platform
- Manual Testing training with QAcart. 

---

 🌐 Connect with me
[![LinkedIn](https://www.linkedin.com/in/wagih-soudi-070233361/?skipRedirect=true)
